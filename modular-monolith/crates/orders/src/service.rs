//! Orders use cases — the heart of the cross-module story.
//!
//! Placing an order requires collaborating with two other modules:
//!   * confirm the customer exists      -> Users
//!   * look up each product's price      -> Catalog
//!
//! Crucially, `OrderService` depends on the *traits* `UserDirectory` and
//! `ProductCatalog`, not on `UserService` / `ProductService`. That is what keeps
//! the modules decoupled and — as the tests below show — independently testable.
...
use crate::domain::{Order, OrderId, OrderLine};
use crate::repository::OrderRepository;

/// One requested line before pricing.
...
impl OrderService {
    ...
    pub async fn place(&self, cmd: PlaceOrder) -> Result<Order, AppError> {
        ...
        // Cross-module call #1: enforce that the customer exists.
        ...
        // Cross-module call #2: price every line via the Catalog contract.
...
mod tests {
    ...
    // Because Orders depends on *traits*, we can test it with tiny fakes and
    // never pull in the real Users or Catalog modules.
    struct FakeUsers {
        exists: bool,
    }
    #[async_trait]
    impl UserDirectory for FakeUsers {
        async fn exists(&self, _id: UserId) -> bool {
            self.exists
        }
    }
    ...
    impl ProductCatalog for FakeCatalog {
        async fn price_of(&self, _id: ProductId) -> Option<u64> {
            self.price
        }
    }
