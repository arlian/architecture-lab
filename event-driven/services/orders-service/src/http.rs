//! Orders' inbound HTTP surface. Unchanged in shape from the microservices
//! version — it maps a request onto the service and returns JSON. The
//! interesting collaboration now happens entirely *behind* the service, in
//! the read model built from events (see read_model.rs / projection.rs).
