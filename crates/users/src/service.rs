//! The application layer: use cases / business rules. This is where invariants
//! are enforced. It depends on the repository *trait*, not a concrete store.
...
/// A command object describing the intent to create a user. Keeping inputs in a
/// dedicated type (instead of loose parameters) keeps the use case readable as
/// it grows.
...
/// The public service that other in-crate code and the HTTP layer call into.
pub struct UserService {
    repo: Arc<dyn UserRepository>,
}

impl UserService {
    pub(crate) fn new(repo: Arc<dyn UserRepository>) -> Self {
        Self { repo }
    }
