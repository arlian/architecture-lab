...
/// "insufficient funds" saga failure (and compensation) path is easy to
/// trigger in the demo (see the README).
    ...
    while let Some(msg) = sub.next().await {
        match serde_json::from_slice::<UserRegistered>(&msg.payload) {
            Ok(evt) => {
                ...
                tracing::info!(
                    "opening wallet for user {user_id} with {STARTING_BALANCE_CENTS}c"
                );
