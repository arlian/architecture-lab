//! Orders' inbound HTTP surface. The one deliberate change versus every
//! other lab: `POST /orders` answers `202 Accepted`, not `200 OK`. Placing an
//! order now only *starts* a saga (see saga.rs) — the order is `Pending`,
//! not `Confirmed`, at the moment this handler returns. Callers poll
//! `GET /orders/:id` to watch it move through the state machine.
